...
SOURCE_CANDIDATES = [
    os.environ.get("EDUKAI_SOURCE_TTF"),
    str(ROOT / "public/fonts/edukai-4.0.ttf"),
    str(Path.home() / "Library/Fonts/edukai-4.0.ttf"),
]
OUTPUT_DIR = ROOT / "public/fonts/edukai"
CSS_FILE = ROOT / ".vitepress/theme/edukai-fonts.css"
TEXT_ROOTS = [
    ROOT / "zh",
...
def source_font() -> Path:
    ...
    raise SystemExit(
        "Could not find edukai-4.0.ttf. Set EDUKAI_SOURCE_TTF=/absolute/path/edukai-4.0.ttf"
    )
...
def font_face(
    ...
    return f'''@font-face {{
  font-family: "{family}";
  src: url("/fonts/edukai/{file_name}") format("woff2");
  font-style: normal;
  font-weight: 400 700;
...
def main() -> None:
    ...
    css_blocks: list[str] = [
        "/* Generated by scripts/build-edukai-subsets.py. */",
    ]
    subset_font(source, OUTPUT_DIR / "edukai-site.woff2", text_file=site_text)
    css_blocks.append(
        font_face(
            "BlogEduKaiSite",
            "edukai-site.woff2",
    ...
    for name, range_value in chunk_ranges():
        file_name = f"edukai-{name}.woff2"
        subset_font(source, OUTPUT_DIR / file_name, unicodes=range_value)
        css_blocks.append(font_face("BlogEduKaiChunked", file_name, range_value))
