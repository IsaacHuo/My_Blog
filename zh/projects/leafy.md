...
MyLeafy 是由我独立开发的通用型校园 iOS 应用。项目的内部代码名、target 和部分类型命名仍沿用 `Leafy`，但产品展示名已经更新为 MyLeafy。当前首个上线校园是北京林业大学，教务连接器直连北林强智教务系统。
...
MyLeafy 从一个朴素的问题出发：为什么查课表、看成绩、找自习室这些校园高频操作，体验还停留在十年前的教务页面？能不能把这件事做得更轻、更优雅？
...
- **教务数据直连学校**，不引入中间层。学号、密码、验证码直接和强智教务系统交互，不经过第三方后端代理。这也意味着需要 APP 自己处理 Cookie 会话、HTML 解析和页面结构变化，用户需要自行刷新。
...
| 层 | 技术 |
|---|---|
| iOS UI | SwiftUI（部署目标不低于 iOS 17.0） |
...
- **`SchoolNetworkManager`** 是教务系统访问的唯一入口，按职责拆为多个扩展（Core / Auth / Timetable / Discover），每个扩展职责需要清晰。
- **展示层预投影**：课表网格布局在进入 SwiftUI body 之前就通过 `TimetableGridSnapshot` 完成计算，避免在渲染热路径上重复过滤、排序和解析备注。这一设计曾解决了 iPhone 16e 上曾出现的 AutoLayout hang 问题，简单说就是卡死。
...
- **3 + N 种可选主题色**：鼠尾草绿（默认）、蒂芙尼蓝、糖果莓粉和自定义颜色
...
学术项目里的数据往往是干净的 JSON API，但真实校园系统是 20 年前的 HTML 页面。因此：
...
主题色、语义色、统一间距和圆角、字体缩放 modifier——这些在最初看起来像是过度设计。但当功能增长到 4 个 Tab、几十个页面后，不硬编码颜色、不手写 magic number 变成了强制力。新页面只需要选择正确的语义 token，深色模式和主题切换自动适配。
